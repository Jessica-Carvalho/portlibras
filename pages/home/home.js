                    ...
                    db.collection("palavrasNivelFacil")
                        ...
                        .then((querySnapshot) => {
                            ...
                            db.collection("jogadores").doc(jogador)
                                ...
                                .then((qr) => {
                                    jogadorDoc = qr.data()
                                    let pontos = criarPontuacao();
                                    pontuacaoPalavras = pontos[0];
                                    pontuacaoJogador = pontos[1];
                                })
...
function criarPontuacao() {
    ...
    if (palavrasFeitasFaceis.length > 0) {
        ...
        pontuacaoPalavras = pontos.reduce((total, atual) => total += atual) + 0;
    }

    if (jogadorDoc.pontuacao != undefined) {
        console.log(jogadorDoc.pontuacao)
        pontuacaoJogador = jogadorDoc.pontuacao;
    }

    pontuacaoFinal = pontuacaoPalavras + pontuacaoJogador;
    ...
    return [pontuacaoPalavras, pontuacaoJogador];
}
...
function comparalistas(letra) {
    ...
    for (i = 0; i < palavraSecretaSorteada.length; i++) {
        if ((palavraSecretaSorteada[i] == letra) && (acertou == true)) {
            ...
            pontuacaoJogador += +1;
            let pontuacaoFinal = pontuacaoPalavras + pontuacaoJogador
            document.getElementById("pontos").innerHTML = 'Pontos: ' + pontuacaoFinal;
        }
    }
    ...
    //Verifica a situação de vitória
    let vitoria = true;
    for (i = 0; i < palavraSecretaSorteada.length; i++) {
    ...
    if ((vitoria == true) && (listaDinamica[i] == palavraSecretaSorteada[i])) {
        ...
        db.collection("jogadores").doc(jogador)
            .set({ pontuacao: pontuacaoJogador }, { merge: true })
            .then(() => db.collection("jogadores/" + jogador + "/palavrasFeitas").doc(palavraSecretaId)
                .set({
                    id: palavraSecretaId,
                    pontos: 5,
